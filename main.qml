...
Item {
    ...
    Plasmoid.fullRepresentation: GridLayout {
        ...
        Layout.minimumWidth: implicitWidth
        //original: Layout.minimumHeight: implicitHeight
		...
		//fix begin
		Layout.minimumWidth: activeMenu.length*8 //calculate button width by string length
		//method.1: It works, but underline words does not show properly(with & before words).  
		//Text {
		//	id: btn
		//	text: activeMenu
		...
		//method.2: Just override the button style, keep text unmodified.
		style: ButtonStyle {
			background: Rectangle {
				id: rect
				//You can customize this place!
				opacity: control.hovered ? 180 : 0
				//color: control.hovered ? "#9e9e9e" : "#ffffff"
				//Behavior on color {
				//	ColorAnimation {
				//		duration: 500
